...
//  TODO: Your dataset filename. Expected format is CSV
  private static final String FILENAME = "src/opt/test/onn-assignment2-full-stdscaler.txt";

  //  TODO: How many examples you have
  private static int num_examples = 11830;

  //  TODO: How many attributes you have. This is (number_of_columns - 1)
  private static int num_attributes = 36;
  ...
  private static int numberHiddenLayerNodes = 5;
	  ...
	  double[][][] attributes = new double[num_examples][][];
		...
		try {
			...
			for (int i = 0; i < attributes.length; i++) {
				...
				attributes[i][0] = new double[num_attributes]; // 58 attributes
		...
		allInstances = new Instance[attributes.length];

		for (int i = 0; i < allInstances.length; i++) {
			allInstances[i] = new Instance(attributes[i][0]);
			allInstances[i].setLabel(new Instance(attributes[i][1][0] == 1 ? 0 : 1)); // values are either 1 or 2
		}
